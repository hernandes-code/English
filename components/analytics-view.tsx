...
export function AnalyticsView(){
  ...
  const evidencePoints=useMemo(()=> (data?.session_series??[]).filter(x=>x.evidence_score!=null).map(x=>({x:x.session_id,y:Number(x.evidence_score)})),[data]);
  const independencePoints=useMemo(()=> (data?.session_series??[]).filter(x=>x.avg_independence!=null).map(x=>({x:x.session_id,y:Number(x.avg_independence)*20})),[data]);
  ...
  return <>
    ...
    <div className="analytics-grid">
      ...
      <Panel className="chart-card"><Eyebrow>INDEPENDENCE</Eyebrow><h2>How independently the language is produced.</h2><p>Average independence per session, normalized to a 0–100 visual scale.</p><SimpleLineChart points={independencePoints} color="var(--cyan)"/></Panel>
