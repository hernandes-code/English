...
export function AnalyticsView(){
  ...
  const evidencePoints=useMemo(()=> (data?.session_series??[]).filter(x=>x.evidence_score!=null).map(x=>({x:x.session_id,y:Number(x.evidence_score),date:x.session_date,displayValue:Number(x.evidence_score).toFixed(1),sampleCount:x.observation_count})),[data]);
  const independencePoints=useMemo(()=> (data?.session_series??[]).filter(x=>x.avg_independence!=null).map(x=>({x:x.session_id,y:Number(x.avg_independence),date:x.session_date,displayValue:`${Number(x.avg_independence).toFixed(2)} / 5`,sampleCount:x.observation_count})),[data]);
  ...
  return <>
    ...
    <div className="analytics-grid">
      <Panel className="chart-card" elevated><Eyebrow>SESSION EVIDENCE SCORE</Eyebrow><h2>How evidence quality is moving.</h2><p>Average scored observations per session on a 0–100 scale. This is not a replacement for Current State; it is a session-level signal.</p><SimpleLineChart points={evidencePoints} metricLabel="Evidence score" yMin={0} yMax={100} ticks={[0,25,50,75,100]}/></Panel>
      <Panel className="chart-card"><Eyebrow>OBSERVED INDEPENDENCE</Eyebrow><h2>How independently the language is produced.</h2><p>Average of observation-level independence ratings per session, shown on the original 1–5 scale.</p><SimpleLineChart points={independencePoints} color="var(--cyan)" metricLabel="Observed independence" yMin={1} yMax={5} ticks={[1,2,3,4,5]}/></Panel>
