...
export function ProfileView(){
  ...
  return <>
    ...
    <div className="profile-grid">
      <Panel className="profile-hero" elevated><PixelAvatar size="lg"/><Eyebrow>BUSINESS ENGLISH PLAYER</Eyebrow><h2>{player.display_name??'Hernandes'}</h2><div className="profile-level">LEVEL {player.level??1}</div><ProgressBar value={Number(player.level_progress_pct??0)}/><div className="profile-stats"><span><Icon name="bolt"/> {player.total_xp??0} XP</span><span><Icon name="coin"/> {player.coin_balance??0} coins</span><span><Icon name="flame"/> {dashboard.current_weekday_streak??0} streak</span></div></Panel>
      <Panel className="profile-learning"><Eyebrow>LEARNING RECORD</Eyebrow><h2>What exists behind the avatar.</h2><div className="record-grid"><Record label="Sessions" value={summary.total_sessions??0}/><Record label="Observations" value={summary.total_skill_observations??0}/><Record label="Practice time" value={analytics ? `${Math.round(Number(analytics.practice?.total_minutes??0))} min` : analyticsError ? 'Unavailable' : 'Loading…'}/><Record label="Current Sprint" value={dashboard.current_sprint?.sprint_id??1}/></div></Panel>
