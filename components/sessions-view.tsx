...
export function SessionsView(){
  ...
  return <>
    ...
    {!loading&&!error&&sessions.length>pageSize&&<nav className="pb-archive-pagination" aria-label="Session history pages"><button disabled={page===0} onClick={()=>setPage(p=>p-1)}>Newer</button><span>Page {page+1} / {Math.ceil(sessions.length/pageSize)}</span><button disabled={(page+1)*pageSize>=sessions.length} onClick={()=>setPage(p=>p+1)}>Older</button></nav>}
